...
import numpy as np
import s0_global1_opt as p1 # Unconstrained
import s0_global2_opt as p2
import s0_global3_opt as p3

# Reading input file

f=open('monteKmod_sol1_def.in',"r") # Params from constrained
lines=f.readlines()
...
n_s0=len(p1.par)
par_p1_in=list(range(n_s0))
# par_p1_in=list(range(n_p))
par_p1=np.take(par, par_p1_in) # # Take params of s0 only


f=open('monteKmod_sol2_def.in',"r") # Idem para 2
lines=f.readlines()
par=[]
for x in lines:
    par.append(float(x.split()[0]))
...
n_s0=len(p2.par)
par_p2_in=list(range(n_s0))
# par_p2_in=list(range(n_p))
par_p2=np.take(par, par_p2_in)

f=open('monteKmod_sol3_def.in',"r") # Idem para 3
lines=f.readlines()
par=[]
for x in lines:
    par.append(float(x.split()[0]))
...
fig, ax = plt.subplots(2,figsize=(16, 24), dpi=100, sharex=False) # Proceed with tf

x=np.linspace(0.28,1.8,200) # sth and smax, datos llegan hasta 1.8
x1=np.linspace(0.28,1.9,200) # datos llegan hasta 1.9 para 1
y=np.array(list(map(lambda z: p1.deltaf2(z**2,par_p1), x1))) # Faster than vectorizing
y2=np.array(list(map(lambda z: p1.deltaf2(z**2,p1.par), x1)))
...
ax[0].fill_between([1.6,2.25], [-5,-5],[480,480],color='k',alpha=0.2,zorder=0)
...
ax[0].text(1.15, 120, '$\\delta^{(0)}_0(s)$ ($^{\\circ}$)',fontsize=48)
...
y=np.array(list(map(lambda z: p1.etaf2(z**2,par_p1), x1)))
y2=np.array(list(map(lambda z: p1.etaf2(z**2,p1.par), x1)))
...
y_2=np.array(list(map(lambda z: p2.etaf2(z**2,par_p2), x)))
y2_2=np.array(list(map(lambda z: p2.etaf2(z**2,p2.par), x)))
...
y_3=np.array(list(map(lambda z: p3.etaf2(z**2,par_p3), x)))
y2_3=np.array(list(map(lambda z: p3.etaf2(z**2,p3.par), x)))
...
ax[1].fill_between(x, errd_2, erru_2,color='tab:red',alpha=0.4,zorder=0)
ax[1].fill_between(x, errd_3, erru_3,color='tab:cyan',alpha=0.5,zorder=0)
ax[1].fill_between([1.6,1.9], [0,0],[1.3,1.3],color='k',alpha=0.2,zorder=0)
ax[1].plot([0.28,2],[1,1],'--',color='grey',zorder=0)
ax[1].plot([1.4,1.4],[0,1.3],'--',color='grey',zorder=0)
