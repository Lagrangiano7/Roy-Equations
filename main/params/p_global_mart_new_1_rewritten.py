"""New P-wave parameterization (piecewise) for Solution I"""
...
par=1.12068779e+00, -8.24261778e-01,  8.66487108e-01,  2.98642329e+00,       -1.63382521e+00, 0.3964268192448001, -0.5915355331455999,  0.2395942655744,       -0.0067205860352,  7.70564904e-01, -6.65942776e+01, -4.95703803e+01,       -3.19890631e+01, -1.07415583e+01, -9.93750424e-01, -1.24490883e-01,        3.23241730e-01,  1.82685636e-01
err_par=N1*0.0035,0.01271903,0.03823714,0.07665504,0.10187444,2*0.00147,0.000827,0.000488,0.000307,0.0012,0.26215286,0.0772043,0.0421058,0.02685989,0.01825,0.00727648+0.0017,0.00265904+0.0017,0.00137747+0.0017
...
def Delta(b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4):
    return (derD(b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho)/derW) + 4*d0 - 9*d1 +16*d2 - 25*d3 +36*d4

def deltaf2(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4):
    return deltaf1(sm,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho) + Delta(b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4)*(c1(w2(s))+1) + d0*(c2(w2(s))-1) + d1*(c3(w2(s))+1) + d2*(c4(w2(s))-1) + d3*(c5(w2(s))+1) + d4*(c6(w2(s))-1) 

def deltaf(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4):
    if s.real<sm:
        return deltaf1(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho)
    else:
        return deltaf2(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4)
...
def tf(s,params):
    b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4,eps2,eps3,eps4=params 
    return (etaf(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,eps2,eps3,eps4)*c.exp(np.pi*1j*deltaf(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4)/90)-1)/(2j*sigma(s,mpi))

def deltafr(s,params):
    b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4,eps2,eps3,eps4=params
    return deltaf(s,b0,b1,b2,b3,b4,k0,k1,k2,k3,mrho,d0,d1,d2,d3,d4)
