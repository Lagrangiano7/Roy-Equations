...
DT_waves = [t.t02_1, t.t04_1, t.t22_1, t.t24_1] # D0, G0, D2, G2

KT = [kernels.K1300, kernels.K1311, kernels.K1320] # S0, P, S2
DT = [kernels.K1302, kernels.K1304, kernels.K1322, kernels.K1324] # D0, G0, F, D2, G2
...
# Diagonal (t13)
Im_t13_vals = np.array([t.t13_1(sp).imag for sp in sp_grid])
Im_t13_interp = PchipInterpolator(sp_grid, Im_t13_vals)
...
def ReT13(s):
    ...
    # DT
    cauchy_kern = lambda sp: kernels.K1313_cauchy(s, sp) * Im_t13_interp(sp)
    cauchy_rest = lambda sp: kernels.K1313_rest(s, sp) * Im_t13_interp(sp)
...
y_teoLo = np.array(list(map(lambda s: ReT13(s**2), x1)))
y_ReggeHI = np.array(list(map(lambda s: Regge.Ret13(s**2), x1)))

y_teo = y_teoLo + y_ReggeHI
y_param = list(map(lambda s: t.t13_1(s**2).real, x1))
