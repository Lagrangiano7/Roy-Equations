...
### Calculating Delta_0 = Ret13-Param13 without changing any parameters
...
# Calculating D_original = param13 - Ret13 (without changing any parameters)
...
for (i, s) in enumerate(x1**2):
    S0_contrib = integrate.quad(lambda sp: k.K1300(s, sp)*Im_t00_interp(sp), sth+eps, s2, limit=200)[0]
    D0_contrib = integrate.quad(lambda sp: k.K1302(s, sp)*Im_t02_interp(sp), sth+eps, s2, limit=200)[0]
    G0_contrib = integrate.quad(lambda sp: k.K1304(s, sp)*Im_t04_interp(sp), sth+eps, s2, limit=200)[0]
    P_contrib = integrate.quad(lambda sp: k.K1311(s, sp)*Im_t11_interp(sp), sth+eps, s2, limit=200)[0]
    F_contrib = integrate.quad(lambda sp: k.K1313_cauchy(s, sp)*Im_t13_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0] + integrate.quad(lambda sp: k.K1313_rest(s, sp)*Im_t13_interp(sp), sth+eps, s2, limit=200)[0]
    S2_contrib = integrate.quad(lambda sp: k.K1320(s, sp)*Im_t20_interp(sp), sth+eps, s2, limit=200)[0]
    D2_contrib = integrate.quad(lambda sp: k.K1322(s, sp)*Im_t22_interp(sp), sth+eps, s2, limit=200)[0]
    G2_contrib = integrate.quad(lambda sp: k.K1324(s, sp)*Im_t24_interp(sp), sth+eps, s2, limit=200)[0]
...
Regge_original = np.array(list(map(lambda s: Regge.Ret13(s**2), x1)))

eval_original = Roy_original + Regge_original
param_original = np.array(list(map(lambda s: t.t13_1(s**2).real, x1)))
...
for i in range(len(par_s0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        S0_plus[j] = integrate.quad(lambda sp: k.K1300(s, sp)*Im_t00_plus_interp(sp), sth+eps, s2, limit=200)[0]
        S0_minus[j] = integrate.quad(lambda sp: k.K1300(s, sp)*Im_t00_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_p_1)):
    ...
    for (j, s) in enumerate(x1**2):
        P_plus[j] = integrate.quad(lambda sp: k.K1311(s, sp)*Im_t11_plus_interp(sp), sth+eps, s2, limit=200)[0]
        P_minus[j] = integrate.quad(lambda sp: k.K1311(s, sp)*Im_t11_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_d0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        D0_plus[j] = integrate.quad(lambda sp: k.K1302(s, sp)*Im_t02_plus_interp(sp), sth+eps, s2, limit=200)[0]
        D0_minus[j] = integrate.quad(lambda sp: k.K1302(s, sp)*Im_t02_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_g0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        G0_plus[j] = integrate.quad(lambda sp: k.K1304(s, sp)*Im_t04_plus_interp(sp), sth+eps, s2, limit=200)[0]
        G0_minus[j] = integrate.quad(lambda sp: k.K1304(s, sp)*Im_t04_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_f_1)):
    ...
    t13_plus = lambda s: t.params1_f1.tf(s, par_plus)
    t13_minus = lambda s: t.params1_f1.tf(s, par_minus)

    Im_t13_plus_interp = interp.getInterp(t13_plus, sp_grid)
    Im_t13_minus_interp = interp.getInterp(t13_minus, sp_grid)

    for (j, s) in enumerate(x1**2):
        F_plus[j] = integrate.quad(lambda sp: k.K1313_rest(s, sp)*Im_t13_plus_interp(sp), sth+eps, s2, limit=200)[0] + integrate.quad(lambda sp: k.K1313_cauchy(s, sp)*Im_t13_plus_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0]
        F_minus[j] = integrate.quad(lambda sp: k.K1313_rest(s, sp)*Im_t13_minus_interp(sp), sth+eps, s2, limit=200)[0] + integrate.quad(lambda sp: k.K1313_cauchy(s, sp)*Im_t13_minus_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0]

    eval_plus = eval_tochange + F_plus
    eval_minus = eval_tochange + F_minus
    
    D_plus = eval_plus - [t13_plus(x).real for x in x1**2]
    D_minus = eval_minus - [t13_minus(x).real for x in x1**2]
...
for i in range(len(par_s2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        S2_plus[j] = integrate.quad(lambda sp: k.K1320(s, sp)*Im_t20_plus_interp(sp), sth+eps, s2, limit=200)[0]
        S2_minus[j] = integrate.quad(lambda sp: k.K1320(s, sp)*Im_t20_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_d2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        D2_plus[j] = integrate.quad(lambda sp: k.K1322(s, sp)*Im_t22_plus_interp(sp), sth+eps, s2, limit=200)[0]
        D2_minus[j] = integrate.quad(lambda sp: k.K1322(s, sp)*Im_t22_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_g2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        G2_plus[j] = integrate.quad(lambda sp: k.K1324(s, sp)*Im_t24_plus_interp(sp), sth+eps, s2, limit=200)[0]
        G2_minus[j] = integrate.quad(lambda sp: k.K1324(s, sp)*Im_t24_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for tag in Regge.params_Regge.keys():
    ...
    for (j, s) in enumerate(x1**2):
        Regge_plus[j] = Regge.Ret13(s, params_Regge_plus)
        Regge_minus[j] = Regge.Ret13(s, params_Regge_minus)
