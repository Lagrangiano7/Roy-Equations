...
STS2 = np.array(list(map(lambda s: k.ST_S2(s**2, k.a00_1, k.a20_1), x1)))
...
for (i, s) in enumerate(x1**2):
    S0_contrib = integrate.quad(lambda sp: k.K2000(s, sp)*Im_t00_interp(sp), sth+eps, s2, limit=200)[0]
    D0_contrib = integrate.quad(lambda sp: k.K2002(s, sp)*Im_t02_interp(sp), sth+eps, s2, limit=200)[0]
    G0_contrib = integrate.quad(lambda sp: k.K2004(s, sp)*Im_t04_interp(sp), sth+eps, s2, limit=200)[0]
    P_contrib = integrate.quad(lambda sp: k.K2011(s, sp)*Im_t11_interp(sp), sth+eps, s2, limit=200)[0]
    F_contrib = integrate.quad(lambda sp: k.K2013(s, sp)*Im_t13_interp(sp), sth+eps, s2, limit=200)[0]
    S2_contrib = integrate.quad(lambda sp: k.K2020_cauchy(s, sp)*Im_t20_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0] + integrate.quad(lambda sp: k.K2020_rest(s, sp)*Im_t20_interp(sp), sth+eps, s2, limit=200)[0]
    D2_contrib = integrate.quad(lambda sp: k.K2022(s, sp)*Im_t22_interp(sp), sth+eps, s2, limit=200)[0]
    G2_contrib = integrate.quad(lambda sp: k.K2024(s, sp)*Im_t24_interp(sp), sth+eps, s2, limit=200)[0]
...
Roy_original = STS2+S0+D0+G0+P+F+S2+D2+G2
Regge_original = np.array(list(map(lambda s: Regge.Ret20(s**2), x1)))

eval_original = Roy_original + Regge_original
param_original = np.array(list(map(lambda s: t.t20_1(s**2).real, x1)))
...
STS2_plus = np.zeros(N_pts)
STS2_minus = np.zeros(N_pts)

eval_tochange = Roy_original - STS2 - S0 + Regge_original
...
for i in range(len(par_s0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        S0_plus[j] = integrate.quad(lambda sp: k.K2000(s, sp)*Im_t00_plus_interp(sp), sth+eps, s2, limit=200)[0]
        S0_minus[j] = integrate.quad(lambda sp: k.K2000(s, sp)*Im_t00_minus_interp(sp), sth+eps, s2, limit=200)[0]
        STS2_plus[j] = k.ST_S2(s, a00_plus, k.a20_1)
        STS2_minus[j] = k.ST_S2(s, a00_minus, k.a20_1)

    eval_plus = eval_tochange + S0_plus + STS2_plus
    eval_minus = eval_tochange + S0_minus + STS2_minus
...
for i in range(len(par_p_1)):
    ...
    for (j, s) in enumerate(x1**2):
        P_plus[j] = integrate.quad(lambda sp: k.K2011(s, sp)*Im_t11_plus_interp(sp), sth+eps, s2, limit=200)[0]
        P_minus[j] = integrate.quad(lambda sp: k.K2011(s, sp)*Im_t11_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_d0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        D0_plus[j] = integrate.quad(lambda sp: k.K2002(s, sp)*Im_t02_plus_interp(sp), sth+eps, s2, limit=200)[0]
        D0_minus[j] = integrate.quad(lambda sp: k.K2002(s, sp)*Im_t02_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_g0_1)):
    ...
    for (j, s) in enumerate(x1**2):
        G0_plus[j] = integrate.quad(lambda sp: k.K2004(s, sp)*Im_t04_plus_interp(sp), sth+eps, s2, limit=200)[0]
        G0_minus[j] = integrate.quad(lambda sp: k.K2004(s, sp)*Im_t04_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_f_1)):
    ...
    for (j, s) in enumerate(x1**2):
        F_plus[j] = integrate.quad(lambda sp: k.K2013(s, sp)*Im_t13_plus_interp(sp), sth+eps, s2, limit=200)[0]
        F_minus[j] = integrate.quad(lambda sp: k.K2013(s, sp)*Im_t13_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
eval_tochange = Roy_original - STS2 - S2 + Regge_original
...
for i in range(len(par_s2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        S2_plus[j] = integrate.quad(lambda sp: k.K2020_cauchy(s, sp)*Im_t20_plus_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0] + integrate.quad(lambda sp: k.K2020_rest(s, sp)*Im_t20_plus_interp(sp), sth+eps, s2, limit=200)[0]
        S2_minus[j] = integrate.quad(lambda sp: k.K2020_cauchy(s, sp)*Im_t20_minus_interp(sp), sth+eps, s2, weight="cauchy", wvar=s, limit=200)[0] + integrate.quad(lambda sp: k.K2020_rest(s, sp)*Im_t20_minus_interp(sp), sth+eps, s2, limit=200)[0]
        STS2_plus[j] = k.ST_S2(s, k.a00_1, a20_plus)
        STS2_minus[j] = k.ST_S2(s, k.a00_1, a20_minus)

    eval_plus = eval_tochange + S2_plus + STS2_plus
    eval_minus = eval_tochange + S2_minus + STS2_minus
    
    D_plus = eval_plus - [t20_plus(s).real for s in x1**2] # New param with +1sigma
    D_minus = eval_minus - [t20_minus(s).real for s in x1**2] # New param with -1sigma
...
for i in range(len(par_d2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        D2_plus[j] = integrate.quad(lambda sp: k.K2022(s, sp)*Im_t22_plus_interp(sp), sth+eps, s2, limit=200)[0]
        D2_minus[j] = integrate.quad(lambda sp: k.K2022(s, sp)*Im_t22_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for i in range(len(par_g2_1)):
    ...
    for (j, s) in enumerate(x1**2):
        G2_plus[j] = integrate.quad(lambda sp: k.K2024(s, sp)*Im_t24_plus_interp(sp), sth+eps, s2, limit=200)[0]
        G2_minus[j] = integrate.quad(lambda sp: k.K2024(s, sp)*Im_t24_minus_interp(sp), sth+eps, s2, limit=200)[0]
...
for tag in Regge.params_Regge.keys():
    ...
    for (j, s) in enumerate(x1**2):
        Regge_plus[j] = Regge.Ret20(s, params_Regge_plus)
        Regge_minus[j] = Regge.Ret20(s, params_Regge_minus)
